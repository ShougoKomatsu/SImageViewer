SImageViewer（本ソフトウェア）は、小松翔悟（作者）が個人の趣味で制作したものであり、このソフトを使ったことにより発生した、いかなる損害に対しても、作者は責任を負いません。
また、作者は本ソフトウェアのソースコードに関する著作権は放棄していません。流用等の相談はGithubコミュニティから行ってください。


================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: SImageViewer プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した SImageViewer には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには SImageViewer アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

SImageViewer.vcxproj
...
SImageViewer.h
   ...
   CSImageViewerApp アプリケーション クラスを宣言します。

SImageViewer.cpp
   これは、アプリケーション クラス CSImageViewerApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\SImageViewer.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル SImageViewer.rc に含まれていま
   す。

res\SImageViewer.rc2
...
SImageViewerDoc.h, SImageViewerDoc.cpp - ドキュメント
   これらのファイルは CSImageViewerDoc クラスを含みます。特別なドキュメント 
   データを追加したり、ファイルを保存したり、
   (via CSImageViewerDoc::Serialize) を読み込んだりするには、これらのファイ
   ルを編集してください。

SImageViewerView.h, SImageViewerView.cpp - ドキュメントのビュー
   これらのファイルは CSImageViewerView クラスを含みます。
   CSImageViewerView オブジェクトは CSImageViewerDoc オブジェクトを表示す
   るのに使用されます。
...
StdAfx.h, StdAfx.cpp
   ...
   SImageViewer.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
